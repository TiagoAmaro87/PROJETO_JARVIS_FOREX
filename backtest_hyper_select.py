...
class HyperSelectAudit:
    ...
    def run(self):
        ...
        print("\n" + "JARVIS HYPER-SELECT AUDIT ($1000 BANK)".center(50, "="))
    ...
    def backtest_symbol(self, symbol):
        ...
        df['ema10'] = df['close'].ewm(span=10).mean() # Faster EMA for pullback
        rsi = self.analyzer.get_rsi_series(df)
        p = mt5.symbol_info(symbol).point
        
        for i in range(200, len(df)-50):
            row = df.iloc[i]
            # --- SUPER SELECTIVE FILTERS ---
            if row['time'].weekday() == 4 and row['time'].hour >= 16: continue
            if not (8 <= row['time'].hour <= 16): continue
            
            # THE LOGIC: Trend + EMA pullback + RSI extreme
            side = ""
            if row['close'] > row['ema200'] and row['low'] <= row['ema10'] and rsi.iloc[i] < 30:
                side = "BUY"
            elif row['close'] < row['ema200'] and row['high'] >= row['ema10'] and rsi.iloc[i] > 70:
                side = "SELL"

            if side:
                # Tight SL (2x ATR) + Aggressive TP (1:4)
                atr = self.analyzer.calculate_atr(df.iloc[:i+1])
                sl_dist = atr * 1.5
                tp_dist = sl_dist * 4.0
                
                trade = {'t': row['time'], 's': symbol, 'p': row['close'], 'res': 'Open', 'ret': 0, 'end_t': None}
    ...
    def report_and_plot(self):
        ...
        print(f"Trades: {len(filtered)} | Win Rate: {wins/len(filtered)*100 if filtered else 0:.1f}% | Balance: ${balance:.2f}")

if __name__ == "__main__":
    HyperSelectAudit(CONFIG_BT).run()
